...
export class CreateDSComponent implements OnInit {
	...
	createDataset() {
		...
		for ( let i = 0; i < this.selectedDatatype.fields.length; i++ ) {
			if ( this.selectedDatatype.fields[i].name == this.selectedKey ) {
				isUuid = true;
			}
		}
			...
			.sendQuery(
				`
					...
					CREATE DATASET ${_dsName} (${this.selectedDatatype.name})
						PRIMARY KEY ${this.selectedKey} ${(isUuid) ? "AUTOGENERATED":""};
				`
			)
			.then(res => {
				res = JSON.parse(res);
